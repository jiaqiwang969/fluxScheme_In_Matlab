...
for j = 2:N-2 % for all faces the domain cells
    ...
    switch fluxMethod
        ...
        case 'AUSMPlusUp' % AUSMPlusUp
            flux(:,j) = AUSMPlusUpflux(qL(:,j),qR(:,j),gamma);
        case 'HLLC' % HLLC
...
switch fluxMethod
    ...
    case 'AUSMPlusUp' % AUSMPlusUp
        flux(:,1) = AUSMPlusUpflux(qL(:,1),qR(:,1),gamma);
    case 'HLLC' % HLLC
...
switch fluxMethod
    ...
    case 'AUSMPlusUp' % AUSMPlusUp
        flux(:,N-1) = AUSMPlusUpflux(qL(:,N-1),qR(:,N-1),gamma);
    case 'HLLC' % HLLC
...
% M.lS. Liou, A sequel to AUSM: AUSM+,
% Journal of Computational Physics 129 (1996) 364-382
...
pu  = -ku * wtl * wtr * fa * af * (rl+rr) * (ur-ul);
...
F(1)  = (mfp*rl    + mfm*rr    )*af;
F(2)  = (mfp*rl*ul + mfm*rr*ur )*af + pf;
F(3)  = (mfp*rl*hl + mfm*rr*hr )*af;
